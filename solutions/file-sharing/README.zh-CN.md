...
预期用途是从已经同步到飞书 / Lark 的在线文档恢复或迁移到指定的本地恢复目录。`scripts/feishu_restore.js` 可以使用本地 `data/state.json`，也可以按 file token 下载云端 `.cloud_server_sync_manifest.json`，再通过 `lark-cli drive +export` 导出已知 docx 文档，并在 dry-run 检查后写入明确指定的 restore root。
...
- 不要提交 `data/.cloud_server_sync_manifest.json`，它是运行期恢复文件
