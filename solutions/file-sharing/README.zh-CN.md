...
预期用途是从已经同步到飞书 / Lark 的在线文档恢复或迁移到指定的本地恢复目录。`scripts/feishu_restore.js` 依赖 `data/state.json`，通过 `lark-cli drive +export` 导出已知 docx 文档，并在 dry-run 检查后写入明确指定的 restore root。
